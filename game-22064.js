...
const CURRENT_VERSION="2.20-beta6.4";
...
function drawIntro(){
  ictx.save();ictx.setTransform(introCanvas.width/INTRO_LOGICAL_W,0,0,introCanvas.height/INTRO_LOGICAL_H,0,0);
  introFrame++;
  // One choreography is about a minute in attract mode; each new cycle gets
  // a different room/route offset so the arcade demo does not repeat identically.
  const scenarioOffset=(introScenario*173+Math.floor(introScenarioSeed))%1080;
  const macroPhase=(introFrame+scenarioOffset)%3600;
  const attractSegment=Math.floor(macroPhase/600); // 6 scenes per minute
  ...
  const ladderA=55+(introScenario*31)%105;
  const ladderB=240+(introScenario*37)%110;
  drawIntroCastleLadder(ladderA,48,96);drawIntroCastleLadder(ladderB,96,144);
...
const FLOOR_HITS=3;
const ladderLayouts=[
  // 1 Entreehal — beginner friendly; two widely separated escape ladders from spawn floor.
  [
    ...
    {x:92,top:312,bottom:372},{x:348,top:312,bottom:372}
  ],
  // 2 Wapenzaal — routes shift, but bottom remains escapable both ways.
  [
    ...
    {x:78,top:312,bottom:372},{x:370,top:312,bottom:372}
  ],
  // 3 Bibliotheek — alternating zig-zag routes.
  [
    ...
    {x:88,top:312,bottom:372},{x:350,top:312,bottom:372}
  ],
  // 4 Kerkers — harder upper routes; safe double exit retained at spawn.
  [
    {x:120,top:64,bottom:126},{x:365,top:64,bottom:126},
    {x:48,top:126,bottom:188},{x:245,top:126,bottom:188},
    {x:190,top:188,bottom:250},{x:415,top:188,bottom:250},
    {x:72,top:250,bottom:312},{x:300,top:250,bottom:312},
    {x:82,top:312,bottom:372},{x:360,top:312,bottom:372}
  ],
  // 5 Troonzaal — broad upper routes, two emergency exits on the bottom.
  [
    ...
    {x:74,top:312,bottom:372},{x:374,top:312,bottom:372}
  ]
]
...
const livingCastle={fogOffset:0,dust:Array.from({length:14},()=>({x:Math.random()*W,y:35+Math.random()*(H-70),vx:(Math.random()-.5)*.05,vy:.02+Math.random()*.04,a:.15+Math.random()*.18})),pebble:null,nextPebble:performance.now()+5000};
...
function drawCastleBackdrop(){
  ...
  if(theme.kind!=="cells"){drawGothicWindow(40,78);drawGothicWindow(220,140);drawGothicWindow(401,78);drawGothicWindow(220,265)}
  ...
  drawCastleTorch(104,98,0);drawCastleTorch(376,98,1.7);drawCastleTorch(155,218,3);drawCastleTorch(325,218,4.4);drawCastleTorch(240,330,2.2);
...
function drawLivingCastle(){if(state!=="play"&&state!=="paused")return;ctx.save();livingCastle.dust.forEach(p=>{p.x+=p.vx;p.y+=p.vy;if(p.y>H-20){p.y=38;p.x=Math.random()*W}ctx.globalAlpha=p.a;ctx.fillStyle="#555";ctx.fillRect(Math.round(p.x),Math.round(p.y),2,2)});const now=performance.now();if(!livingCastle.pebble&&now>livingCastle.nextPebble)livingCastle.pebble={x:40+Math.random()*(W-80),y:34,vy:.75};if(livingCastle.pebble){livingCastle.pebble.y+=livingCastle.pebble.vy;ctx.globalAlpha=.7;ctx.fillStyle="#444";ctx.fillRect(livingCastle.pebble.x,livingCastle.pebble.y,3,3);if(livingCastle.pebble.y>H-20){livingCastle.pebble=null;livingCastle.nextPebble=now+6000+Math.random()*9000}}ctx.restore()}
