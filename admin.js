  ...
  playerList.querySelectorAll("[data-delete-player]").forEach(btn=>{
    btn.addEventListener("click",async e=>{
      ...
      try{
        ...
        console.error(err);
        alert("Verwijderen is mislukt. Controleer of de v2.26-beta.3.1 SQL-migratie is uitgevoerd.");
        btn.disabled=false;
...
function renderLevels(list){
  ...
  }).join(""):"<div class='small emptyBox'>Nog geen level-events geregistreerd. Speel v2.26-beta.3.1 om deze data te vullen.</div>";
}
...
function renderEvents(list){
  ...
  `).join(""):"<div class='small emptyBox'>Nog geen v2.26-beta.3.1-events.</div>";
}
...
async function loadV222Analytics(){
  ...
  }catch(e){el.textContent="v2.26-beta.3.1 analytics nog niet beschikbaar — controleer SQL 007.";}
}
loadV222Analytics();
