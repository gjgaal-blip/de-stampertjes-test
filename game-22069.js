...
function showLevelTransition(completedLevel,nextLevel){
  ...
  levelSubtitle.textContent=`NIEUWE ZAAL — ${CASTLE_ROOM_THEMES[(nextLevel-1)%CASTLE_ROOM_THEMES.length].name}`;
  levelOverlay.classList.remove("hidden");

  setTimeout(()=>{
    const nextRoom=CASTLE_ROOM_THEMES[(nextLevel-1)%CASTLE_ROOM_THEMES.length];
...
const CURRENT_VERSION="2.20-beta6.9";
...
activateButton(versionLabel,()=>{
  ...
  if(teddyTapCount>=5){
    ...
    updatePlayerContextOnline();
    logGameEvent("teddy_found",{level,score});
...
function drawIntro(){
  ...
  // Beta 6.5: four visible floors, matching the taller main game.
...
const livingCastle={
  ...
  teddy:null,nextTeddy:performance.now()+18000+Math.random()*26000,
...
function drawLivingCastle(){
  ...
  // Teddy is deliberately rare and only watches from the background.
  if(!livingCastle.teddy&&now>livingCastle.nextTeddy&&Math.random()<.015){
    const spots=[[48,101],[228,163],[409,101],[228,288]];
    livingCastle.teddy={...Object.fromEntries([["spot",spots[Math.floor(Math.random()*spots.length)]],["until",now+2600]])};
  }
  if(livingCastle.teddy){
    if(now>livingCastle.teddy.until){
      livingCastle.teddy=null;livingCastle.nextTeddy=now+25000+Math.random()*45000;
      livingCastle.windowShift=Math.floor(Math.random()*4);
    }else{
      const [tx,ty]=livingCastle.teddy.spot;
      ctx.globalAlpha=.7;ctx.fillStyle="#222";ctx.font="15px monospace";ctx.fillText("ᓚᘏᗢ",tx,ty);
    }
  }
  ctx.restore();
}
