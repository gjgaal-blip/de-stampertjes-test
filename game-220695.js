...
const CURRENT_VERSION="2.20-beta6.9.5";
...
function startGame(){
  ...
  livingCastle.nextTeddy=performance.now()+25000+Math.random()*25000;
  spawnLevel();
...
function spawnLevel(){
  ...
  if(!livingCastle.teddyCelebrating)livingCastle.nextTeddy=Math.max(livingCastle.nextTeddy,performance.now()+12000);
  keys.left=keys.right=keys.up=keys.down=false;
  pauseOverlay.classList.add("hidden");
  pauseToggle.textContent="⏸ PAUZE";
...
const livingCastle={
  ...
  teddy:null,nextTeddy:performance.now()+25000+Math.random()*25000,teddyCelebrating:false,teddyMusicWasPlaying:false,
...
function finishTeddyMoment(){
  livingCastle.teddy=null;
  livingCastle.teddyCelebrating=false;
  livingCastle.nextTeddy=performance.now()+45000+Math.random()*45000;
...
function triggerTeddyMoment(){
  ...
  if(!Array.isArray(teddyStats.achievements))teddyStats.achievements=[];
  if(!teddyStats.achievements.includes("teddy"))teddyStats.achievements.push("teddy");
  saveStats(teddyStats);
  updatePlayerContextOnline();
  logGameEvent("teddy_found",{level,score});
...
function updateGameplayTeddy(){
  ...
  if(!livingCastle.teddy){
    if(state==="play"&&!livingCastle.teddyCelebrating&&now>=livingCastle.nextTeddy){
      spawnGameplayTeddy();
    }
    return;
  }
  ...
  if(now>=teddy.expiresAt){
    livingCastle.teddy=null;
    livingCastle.nextTeddy=now+45000+Math.random()*45000;
    return;
  }
